...
otu.MY.tmp <- read.table("../../Data/16S/otu.16S.2400.MY.csv", header=TRUE, sep=',', row.names = 1)
otu.MY <- otu.MY.tmp[, apply(otu.MY.tmp, 2, sum) > 0]/2400 # remove OTUs with no seqs and divide by 2400 to get relative abundance (0-1)

### Load the taxonomy information into the environment
tax.MY <- read.table("../../Data/16S/taxonomy.16S.MY.csv", header=TRUE, sep=',', row.names = 1)

### Load the weighted UniFrac distance matrices into the environment
w.unifrac.MY.tmp <- read.table("../../Data/16S/w.unifrac.16S.MY.csv", header=TRUE, sep=',', row.names = 1)
w.unifrac.MY <- as.matrix(w.unifrac.MY.tmp)

### Load the environmental metadata tables with alpha diversity into the environment and set factors
env.MY <- read.table("../../Data/16S/env.16S.2400.MY.csv", header=TRUE, sep=',', row.names = 1)
...
row.names(w.unifrac.MY) == row.names(env.MY)
...
bargraph.CI(Compartment, PD, Banana, data = env.MY, col = mypal.banana(3), legend = T) 
...
#adonis2(w.unifrac.MY ~ Compartment * Banana, data = env.MY, method = 'euc')
...
hm.tmp <- otu.MY.mean[,which(apply(otu.MY.mean,2,max)>= 0.005)]
